...
private:
    ...
    long double sigmaXiMinusXBarSquared, sigmaYiMinusYBarSquared;

    // these variable names are getting fucking large
    long double *xiMinusXBarTimesYiMinusYBar;
    long double sigmaXiMinusXBarTimesYiMinusYBar;

    // variances, standard deviations, covariance
    // using long double for more accuracy
    long double xVariance, yVariance;
    long double xStdDev, yStdDev;
    ...
    long double intercept, slope, error;
    ...
    // SST = SSE + SSR
    long double SSE, SSR, SST;

    // coefficient of determination, MSE, MSESquared, slopeStdDev
    long double coefficientOfDetermination;
    long double MSE, MSERoot;
    long double slopeStdDev;

    // t test statistic POGGERRRRSSSS
    long double tTestStatistic;

public:
    LinearRegression()
    {
        // initialize everything to 0 for safety
        // they'll either be incremented or re-assigned, so zero is a safe bet
        xBar = 0;
        yBar = 0;
        popSize = 0;
        sigmaXiMinusXBarSquared = 0, sigmaYiMinusYBarSquared = 0;
        sigmaXiMinusXBarTimesYiMinusYBar = 0;
        xVariance = 0, yVariance = 0;
        xStdDev = 0, yStdDev = 0;
        covariance = 0;
        correlationCoefficient = 0;
        intercept = 0, slope = 0, error = 0;
        SSE = 0, SSR = 0, SST = 0;
    ...
    long double
    calculateMean(int popSize, long double *pop)
    {
    ...
    ~LinearRegression()
    {
        // free memory so ram don't go boomie
        delete xValues;
        delete yValues;
        delete xiMinusXBar;
        delete yiMinusYBar;
